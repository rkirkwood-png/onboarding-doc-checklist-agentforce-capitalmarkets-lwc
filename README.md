...
## FlexCard + Agentforce Agent Extension

This repository also includes the **MADocumentChecklist2 FlexCard**, the **analyzeDocumentOnboarding LWC**, the **Market Axess Onboarding Agent**, and the **Onboarding_Document_Analysis Prompt Template** — an extension that adds a FlexCard-based document checklist view to the Case record page with one-click Agentforce document analysis.

### Additional Components

| Component | Type | Purpose |
|---|---|---|
| `MADocumentChecklist2` | OmniStudio FlexCard | Renders DocumentChecklistItem rows with status, upload, open, and Analyze Document actions |
...
### Deploy the Additional Components

**1. Deploy Flow, Prompt Template, Agent, and LWC via CLI:**
```bash
sf project deploy start \
...
**2. Configure Agent Topics & Actions (Manual)**

`GenAiPlanner` metadata is not deployable via CLI in API 66.0. In **Setup → Agents → Market Axess Onboarding Agent → Agent Builder**:
- Create a **Document Analysis** topic
- Add a Flow action pointing to `Onboarding_Document_Checklist_Item_Info_Collection`
- Instruct the agent to extract the `DocumentChecklistItemId` from the user message

**3. Update the Bot ID in the LWC**

Open `force-app/main/default/lwc/analyzeDocumentOnboarding/analyzeDocumentOnboarding.js` and replace `BOT_ID` with your org's agent ID (found in the Agent Builder URL):
```javascript
const BOT_ID = '0Xxak000003JDz7CAG'; // ← replace with your org's Bot ID
```

**4. Import the FlexCard**

In **Setup → OmniStudio → FlexCards → Import**, upload `flexcard/MADocumentChecklist2.json` and Activate. Then add a Custom LWC element to the ItemRow block: name `analyzeDocumentOnboarding`, attribute `recordId → {Id}`.
